...
// --- ICONS & LABELS CONFIGURATION ---
const sidebarLinks = [
  { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard },
  { id: 'orders', label: 'My Orders', icon: Package },
  { id: 'wishlist', label: 'Wishlist', icon: Heart },
  { id: 'addresses', label: 'Addresses', icon: MapPin },
  { id: 'coupons', label: 'Coupons', icon: Tag },        // Changed Ticket to Tag
  { id: 'rewards', label: 'Rewards', icon: Trophy },    // Changed Award to Trophy
  { id: 'reviews', label: 'Reviews', icon: Star },
  { id: 'measurements', label: 'Measurements', icon: Move }, // Changed Ruler to Move
...
export default function Profile() {
  ...
  const DashboardTab = () => (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-12">
        ...
        <StatCard title="Coupons" value="0" icon={Ticket} color="bg-green-100 text-green-600" />
        <StatCard title="Reward Points" value="1,250" icon={Award} color="bg-purple-100 text-purple-600" />
      </div>
            ...
            <motion.div key={order.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-6 border border-border rounded-xl bg-white dark:bg-gray-800 hover:shadow-md transition-shadow">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4 pb-4 border-b border-gray-100 dark:border-gray-700">
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-xs font-bold uppercase px-3 py-1 rounded-full ${order.status === 'CONFIRMED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                    {order.status}
                  </span>
          ...
          {wishlist.map(item => (
            <motion.div key={item.id} whileHover={{ y: -5 }} className="group border border-border rounded-xl overflow-hidden bg-white dark:bg-gray-800 hover:shadow-lg transition-all duration-300">
              ...
              <div className="p-4">
                ...
                <button className="w-full mt-3 border border-foreground py-2 text-[11px] font-semibold uppercase tracking-luxe-sm hover:bg-foreground hover:text-white transition-colors">
                  Move to Cart
                </button>
              </div>
            </motion.div>
  ...
  // --- PERFECTLY PRESERVED SETTINGS TAB WITH EXACT LOGIC ---
  const SettingsTab = () => {
    const [settingsView, setSettingsView] = useState('info'); // 'info' or 'password'
    ...
    return (
      <div className="max-w-2xl">
        ...
        <div className="flex gap-6 mb-8 border-b border-border">
          <button onClick={() => setSettingsView('info')} className={`pb-3 text-sm font-medium border-b-2 transition-colors ${settingsView === 'info' ? 'border-foreground text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
            Profile Information
          </button>
          <button onClick={() => setSettingsView('password')} className={`pb-3 text-sm font-medium border-b-2 transition-colors ${settingsView === 'password' ? 'border-foreground text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
            Change Password
          </button>
        </div>

        {/* Info Tab (Upgraded UI) */}
        {settingsView === 'info' && (
        ...
        {/* Password Tab (Exact Original Logic) */}
        ...
        <div className="flex gap-10">
          ...
          <div className="flex-1 lg:ml-0 min-h-screen">
            <AnimatePresence mode="wait">
              ...
              >
                {activeTab === 'dashboard' && <ProfileHeader />}
